(function (global, factory) {
    ...
    var utils_js__utils = {
        ...
        API_VERSION: '3',
        CLIENT_ID: '',
        buildOptions: function buildOptions(apiUrl, path, method, cb) {
            return { apiUrl: apiUrl, path: path, method: method, cb: cb };
        }
    };
    ...
    var ImgurMethod__imgurMethod = function ImgurMethod__imgurMethod(options) {
        ['cb', 'method', 'apiUrl', 'path'].forEach(function (option) {
    ...
    var imageEndpoint = ImgurEndpoint({
        ...
        get: function get(hash, cb) {
            var options = utils_js.buildOptions(this.apiUrl, [this.path, hash].join('/'), 'get', cb);
    ...
    var oauth2Endpoint = ImgurEndpoint({
        ...
        get: function get(responseType, cb) {
            ...
            var options = utils_js.buildOptions(this.apiUrl, path, 'get', cb);

            return this.imgurMethod(options);
        },
        refresh: function refresh(refreshToken, clientSecret, cb) {
            ...
            var options = utils_js.buildOptions(this.apiUrl, path, 'post', cb);
